...
import { arduinoAnalyzerShimLineOffset, arduinoAnalyzerShimSource } from './firmware-shim-source.mjs';
import { stripResolvedFirmwareIncludes } from './library-resolver.mjs';
...
function arduinoShim(code) {
  ...
  return `${arduinoAnalyzerShimSource()}\n${inferredAliasSource(source)}${stripResolvedFirmwareIncludes(source)}\n`;
}

function shimLineOffset() {
  return arduinoAnalyzerShimLineOffset();
}
