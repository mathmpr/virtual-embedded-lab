...
test('web UI entrypoint contains required workspace regions', () => {
  ...
  assert.match(html, /id="openNewProject"/);
  assert.match(html, /id="newProjectDialog"/);
  assert.match(html, /id="newProjectName"/);
  assert.match(html, /id="newProjectDescription"/);
  assert.match(html, /data-new-project-cancel/);
  assert.match(html, /data-new-project-create/);
  assert.doesNotMatch(html, /id="newProjectForm" method="dialog"/);
  assert.match(html, /id="currentProjectTitle"/);
  assert.match(html, /id="simulationNoticeDialog"/);
  assert.match(html, /id="ackSimulationNotice"/);
  assert.match(html, /data-i18n="Simulation notice intro"/);
  assert.match(html, /data-i18n="I understand"/);
  assert.doesNotMatch(html, /simulationNoticeDialog[\s\S]*value="cancel"/);
  assert.match(html, /id="openExamples"/);
  assert.match(html, /id="examplesDialog"/);
  assert.match(html, /id="exampleSearch"/);
...
test('web server exposes official component and example catalog APIs', () => {
  ...
  assert.match(server, /boardGroup/);
  assert.match(server, /descriptionI18n/);
...
test('web UI prevents known interaction regressions', () => {
  ...
  assert.match(script, /loadExampleById\('esp32-s3-snake-hub75', false\)/);
  assert.doesNotMatch(script, /loadExampleById\('hc-sr04-led-distance', false\)/);
  assert.match(script, /showSimulationNoticeIfNeeded\(\)/);
  assert.match(script, /virtualEmbeddedLabSimulationNoticeAccepted/);
  assert.match(script, /dialog\.addEventListener\('cancel'/);
  assert.match(script, /event\.preventDefault\(\)/);
  assert.match(script, /Max-Age/);
  assert.match(script, /SameSite=Lax/);
  assert.match(script, /renderExampleGrid\(examples, examplesList, search\)/);
  assert.match(script, /renderExampleDetail\(example, examples, examplesList, search\)/);
  assert.match(script, /exampleWithDetails\(example\)/);
  assert.match(script, /localizedExampleDescription\(example\)/);
  assert.match(script, /descriptionI18n\?\.\[getLocale\(\)\]/);
  assert.match(script, /loadExampleProject\(example\.id\)/);
  assert.match(script, /typeByComponentId\[componentId\]/);
  assert.match(script, /example\.name\.toLowerCase\(\)\.includes\(query\)/);
  assert.match(script, /search\.hidden = true/);
  assert.match(script, /data-example-back/);
  assert.match(script, /data-example-load/);
